...
const suitImages: Record<Suit, string> = {
  ...
  Yellow: 'https://tjftzpjqfqnbtvodsigk.supabase.co/storage/v1/object/public/d33m_images/epl_players/223723.png',
  Blue: 'https://tjftzpjqfqnbtvodsigk.supabase.co/storage/v1/object/public/d33m_images/epl_players/223723.png',
  Green: 'https://tjftzpjqfqnbtvodsigk.supabase.co/storage/v1/object/public/d33m_images/epl_players/223723.png',
  Purple: 'https://tjftzpjqfqnbtvodsigk.supabase.co/storage/v1/object/public/d33m_images/epl_players/223723.png'
};
...
const GameDrawCards: React.FC<DrawCardsProps> = ({ onCardDraw }) => {
  ...
  const renderCards = (suit: Suit) => {
    const maxCascadingCards = 5;
    return cardStacks[suit].map((card, index) => {
      ...
      return (
        <div key={index} className="absolute w-14 h-24 border border-black flex justify-center items-center bg-notWhite rounded-lg" style={positionStyle}>
      ...
      <div className="flex flex-wrap justify-center">
        {(['Red', 'Yellow', 'Blue', 'Green', 'Purple'] as Suit[]).map(suit => (
          <div key={suit} className="flex flex-col items-center">
            {/* Suit Image */}
            <div className={`mr-2 w-24 h-36 border border-black flex justify-center items-center cursor-pointer`} onClick={() => handleSuitTap(suit)}>
              <img src={suitImages[suit]} alt={`${suit} card`} className="w-full h-full object-cover rounded-lg border-2 border-gray-800 bg-notWhite" />
            </div>
